...
namespace DotNet4.Utilities
{
    ...
    public class HttpHelper
    {
        ...
        private void SetCer(HttpItem item)
        {
            if (!string.IsNullOrWhiteSpace(item.CerPath))
            {
                ...
                if(item.SecurityProtocol != default(SecurityProtocolType))
                {
        ...
        {
            request.CookieContainer = item.CookieContainer ?? new CookieContainer();
        ...
        public static string GetAllCookieString(CookieContainer cc)
        {
            ...
            {
                str = str + str2 + cookie.Name + "=" + cookie.Value;
                str2 = "; ";
            }
            return str;
        }

        public static string UpdateCookie(string cookieOld, string cookieNew)
        {
            StringBuilder sb = new StringBuilder();

            Dictionary<string, string> dicCookie = new Dictionary<string, string>();
            //遍历cookie1
            if (!string.IsNullOrEmpty(cookieOld))
            {
                foreach (string cookie in cookieOld.Replace(',', ';').Split(';'))
                {
            ...
            if (!string.IsNullOrEmpty(cookieNew))
            {
                //遍历cookie2
                foreach (string cookie in cookieNew.Replace(',', ';').Split(';'))
                {
                    if (!string.IsNullOrEmpty(cookie) && cookie.IndexOf('=') > 0)
                    {
                        string key = cookie.Split('=')[0].Trim();
    ...
    public class HttpItem
    {
        ...
        private SecurityProtocolType _SecurityProtocol = default(SecurityProtocolType);
        /// <summary>
        /// HTTPS协议类型
        /// </summary>
